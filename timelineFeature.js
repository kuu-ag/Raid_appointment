...
function extractTimelineDate(row) {
  const dateText = row.date || row.regDate || row.time || row.timelineDate || row.createdDate || "";
  if (!dateText) return "";
  ...
  if (compact) return `${compact[1]}-${compact[2]}-${compact[3]} 00:00`;
...
function classifyTimeline(row) {
  ...
  let logType = "기타";
  if (text.includes("획득") || text.includes("itemName")) logType = "아이템";
  if (text.includes("클리어") || text.includes("처치") || text.includes("성공")) logType = "클리어";
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .card { border: 1px solid #272b3a; border-radius: 16px; padding: 16px; background: #111827; margin-bottom: 16px; }
    .card h2 { margin: 0 0 12px; font-size: 17px; }
    label { display: block; margin: 12px 0 6px; color: #d4d4d8; font-size: 13px; font-weight: 800; }
    input, select, textarea { width: 100%; border: 1px solid #34394d; border-radius: 10px; padding: 10px; background: #080b12; color: #fff; outline: none; }
    textarea { min-height: 150px; resize: vertical; }
    button, .btn { display: inline-flex; align-items: center; justify-content: center; border: 0; border-radius: 10px; padding: 10px 12px; background: #7c3aed; color: #fff; font-weight: 900; cursor: pointer; }
    ...
    th, td { border-bottom: 1px solid #272b3a; padding: 10px 8px; vertical-align: top; }
    th { text-align: left; color: #a1a1aa; font-size: 12px; }
    .badge { display: inline-flex; padding: 4px 7px; border-radius: 999px; background: #27272a; color: #d4d4d8; font-size: 11px; font-weight: 800; }
    ...
    .log { border: 1px solid #272b3a; border-radius: 14px; padding: 12px; margin-bottom: 10px; background: #0d111a; }
    .logTop { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 8px; }
    ...
    .stat { flex: 1 1 120px; border: 1px solid #272b3a; border-radius: 12px; padding: 12px; background: #0d111a; }
...
function registerTimelineFeature(app, db, options = {}) {
  ...
  app.get(`${ADMIN_BASE}/timeline`, requireAdmin, (req, res) => {
    ...
    if (refreshState?.last_refreshed_at) {
      ...
      if (Number.isFinite(lastTime) && diff < REFRESH_COOLDOWN_MS) {
        cooldownActive = true;
        const remainSec = Math.ceil((REFRESH_COOLDOWN_MS - diff) / 1000);
        cooldownText = `쿨타임 ${Math.floor(remainSec / 60)}분 ${remainSec % 60}초 남음`;
      } else {
        cooldownText = `마지막 갱신: ${refreshState.last_refreshed_at}`;
      }
    }
    ...
    res.send(layout("캐릭터 통합 타임라인", `
      <div class="top">
        ...
        <div class="row"><a class="btn btnSub" href="${ADMIN_BASE}">관리자 홈</a></div>
      </div>
      ...
      <div class="grid">
        <aside>
          ...
          <section class="card">
            <h2>타임라인 갱신</h2>
            <p class="muted">활성화된 모든 캐릭터를 한 번에 갱신합니다.<br>수동 갱신은 5분 쿨타임이 적용됩니다.<br>${escapeHtml(cooldownText)}</p>
            <form method="post" action="${ADMIN_BASE}/timeline/refresh" onsubmit="return confirm('활성화된 모든 캐릭터의 타임라인을 갱신할까요?')">
              ...
              <div style="margin-top:12px"><button type="submit" ${cooldownActive ? "disabled" : ""}>전체 캐릭터 갱신</button></div>
            </form>
          </section>
          ...
          <section class="card">
            <h2>등록 캐릭터</h2>
            <table>
              <thead><tr><th>서버</th><th>캐릭터</th><th>상태</th><th>갱신</th><th>관리</th></tr></thead>
        ...
        <main>
          ...
          <section>${logRows || `<div class="card muted">표시할 타임라인 로그가 없습니다.</div>`}</section>
        </main>
      </div>
    `));
  });
  ...
  app.post(`${ADMIN_BASE}/timeline/characters/add`, requireAdmin, async (req, res) => {
    try {
      ...
      if (!serverId || !characterName) return res.status(400).send("server_id와 character_name이 필요합니다.");

      const serverName = SERVER_OPTIONS.find(([id]) => id === serverId)?.[1] || serverId;
      const found = await searchCharacter(serverId, characterName);
      if (!found) return res.status(404).send(`캐릭터를 찾지 못했습니다: ${escapeHtml(characterName)}`);

      db.prepare(`
        INSERT OR IGNORE INTO tracked_characters
          (server_id, server_name, character_id, character_name, memo, enabled)
        VALUES (@server_id, @server_name, @character_id, @character_name, @memo, 1)
      `).run({
        server_id: serverId,
  ...
  app.post(`${ADMIN_BASE}/timeline/characters/bulk-add`, requireAdmin, async (req, res) => {
    ...
    for (const line of lines) {
      try {
        ...
        db.prepare(`
          INSERT OR IGNORE INTO tracked_characters
            (server_id, server_name, character_id, character_name, memo, enabled)
          VALUES (@server_id, @server_name, @character_id, @character_name, '', 1)
        `).run({
          server_id: serverId,
          server_name: serverName,
          character_id: found.characterId,
    ...
    res.send(layout("일괄 등록 결과", `
      <div class="top"><div><h1>일괄 등록 결과</h1><p class="desc">등록 결과입니다.</p></div><a class="btn" href="${ADMIN_BASE}/timeline">돌아가기</a></div>
      <section class="card"><pre style="white-space:pre-wrap; line-height:1.6">${escapeHtml(results.join("\n"))}</pre></section>
    `));
  });
  ...
  app.post(`${ADMIN_BASE}/timeline/characters/:id/toggle`, requireAdmin, (req, res) => {
    ...
    if (row) db.prepare(`UPDATE tracked_characters SET enabled = ? WHERE id = ?`).run(row.enabled ? 0 : 1, id);
    res.redirect(`${ADMIN_BASE}/timeline`);
  });
  ...
  app.post(`${ADMIN_BASE}/timeline/characters/:id/delete`, requireAdmin, (req, res) => {
    ...
    const row = db.prepare(`SELECT * FROM tracked_characters WHERE id = ?`).get(id);
    if (row) {
      db.prepare(`DELETE FROM character_timeline_logs WHERE server_id = ? AND character_id = ?`).run(row.server_id, row.character_id);
  ...
  app.post(`${ADMIN_BASE}/timeline/refresh`, requireAdmin, async (req, res) => {
    ...
    const refreshState = db.prepare(`SELECT last_refreshed_at FROM timeline_refresh_state WHERE id = 1`).get();
    if (refreshState?.last_refreshed_at) {
      const lastTime = new Date(`${refreshState.last_refreshed_at}Z`).getTime();
      const diff = Date.now() - lastTime;
      if (Number.isFinite(lastTime) && diff < REFRESH_COOLDOWN_MS) {
        ...
        return res.status(429).send(layout("타임라인 갱신 대기", `
          <div class="top"><div><h1>타임라인 갱신 대기</h1><p class="desc">수동 갱신은 5분마다 1회만 실행할 수 있습니다.</p></div><a class="btn" href="${ADMIN_BASE}/timeline">돌아가기</a></div>
          <section class="card"><h2>아직 쿨타임이 남아있습니다.</h2><p class="muted">남은 시간: ${Math.floor(remainSec / 60)}분 ${remainSec % 60}초<br>마지막 갱신 시각: ${escapeHtml(refreshState.last_refreshed_at)}</p></section>
        `));
      }
    }
    ...
    const targets = db.prepare(`
      ...
      FROM tracked_characters
      WHERE enabled = 1
      ORDER BY CASE WHEN last_refreshed_at IS NULL THEN 0 ELSE 1 END, last_refreshed_at ASC, id ASC
    `).all();

    if (targets.length === 0) {
      return res.send(layout("타임라인 갱신 결과", `
        <div class="top"><div><h1>타임라인 갱신 결과</h1><p class="desc">갱신할 활성 캐릭터가 없습니다.</p></div><a class="btn" href="${ADMIN_BASE}/timeline">돌아가기</a></div>
      `));
    }
    ...
    const updateRefreshed = db.prepare(`UPDATE tracked_characters SET last_refreshed_at = CURRENT_TIMESTAMP WHERE id = ?`);
    ...
    res.send(layout("타임라인 갱신 결과", `
      <div class="top"><div><h1>타임라인 갱신 결과</h1><p class="desc">${escapeHtml(startDate)} ~ ${escapeHtml(endDate)} / 활성 캐릭터 ${targets.length}명 전체 갱신</p></div><a class="btn" href="${ADMIN_BASE}/timeline">돌아가기</a></div>
      <div class="pillbar">
      ...
      <section class="card"><h2>성공</h2><pre style="white-space:pre-wrap; line-height:1.6">${escapeHtml(inserted.join("\n") || "없음")}</pre></section>
      <section class="card"><h2>실패</h2><pre style="white-space:pre-wrap; line-height:1.6">${escapeHtml(failed.join("\n") || "없음")}</pre></section>
    `));
  });
}
